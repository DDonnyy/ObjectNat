...
def is_shown(
    ...
    assert selection_zone.crs == buildings.crs == services.crs == links.crs, "CRS mismatch"
...
def recalculate_links(
    ...
    free_demand['distance'] = free_demand.apply(lambda x: sum([(x1*x2) for x1,x2 in zip(x.demand,x.distance)]),axis=1)
    free_demand['demand'] = free_demand['demand'].apply(sum)
    free_demand = free_demand.reindex(buildings.index, fill_value=0)
    new_sum_time = (buildings['supplyed_demands_within']+buildings['supplyed_demands_without'])*buildings['avg_dist'] - free_demand['distance']
    ...
    buildings['avg_dist'] = new_sum_time / (buildings['supplyed_demands_without']+buildings['supplyed_demands_within'])
