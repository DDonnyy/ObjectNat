...
def get_clusters_polygon(points: gpd.GeoDataFrame, min_dist=100, min_point=5) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    """
    ...
    if len(services_normal) > 0:
        cluster_service = services_normal.groupby("cluster", group_keys=True).apply(lambda x: _get_service_ratio(x))
    ...
    if len(services_outlier) > 0:
        ...
        cluster_service = services_outlier.groupby("cluster", group_keys=True).apply(lambda x: _get_service_ratio(x))
        if isinstance(cluster_service, pd.Series):
            cluster_service = cluster_service.unstack(level=1, fill_value=0)
